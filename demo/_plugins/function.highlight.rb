...
eos
      ...
      def example(output)
        "<div class=\"demo-container\"><div class=\"demo-container--example\">\n#{output}\n</div>"
      end

      def remove_holderjs(code)
        code = code.gsub(/src="holder.js.+?"/, 'src="..."')
      end

      def remove_example_classes(code)
        # Find `demo-` classes and remove them from the highlighted code. Because of how this regex works, it will also
        # remove classes that are after the `demo-` class. While this is a bug, I left it because it can be helpful too.
        # To fix the bug, replace `(?=")` with `(?=("|\ ))`.
        code = code.gsub(/data-src=".*" /, "")
        code = code.gsub(/(?!class=".)\ *?demo-.+?(?=")/, "")
      ...
      def render_rouge(code)
        ...
        "<div class=\"demo-container--code highlight\"><pre>#{code}</pre></div></div>"
      end
